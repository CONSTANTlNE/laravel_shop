    ...
    <div class="col-6">
        <a href="{{route('product.single',[app()->getLocale(),$search_product->slug])}}">
            <div class="card card-style custom-card m-0 bg-21"
                 ...
                 style="height: 140px;
                                     @if($search_product->getFirstMedia('thumbnail'))
                                         background-image: url({{$search_product->getFirstMediaUrl('thumbnail')}})
                                     @else
            ...
            <div class="d-flex justify-content-center align-items-center gap-2">
                <div class="d-flex justify-content-center gap-2">
                    ...
                    @if($search_product->price_before_discount && $site_settings->show_discounted==1)
                        <del class="opacity-50 font-15 mt-n1">
                            {{$search_product->price_before_discount }}₾
                        </del>
                    @endif
                </div>
            </div>
        ...
        <div class="d-flex justify-content-center">
            <button
                @if($search_product->in_stock==1)
                data-toast="cart_toast"
                hx-post="{{route('cart.add')}}"
                hx-target="#cart_icon_number"
                hx-vals='{"product_id":"{{$search_product->id}}","_token":"{{csrf_token()}}","is_cart":"0"}'
                @endif
                class="{{$search_product->in_stock==1 ? 'gradient-highlight' : 'gradient-red' }} btn-full btn shadow-bg shadow-bg-m ">
                @if($search_product->in_stock==1)
