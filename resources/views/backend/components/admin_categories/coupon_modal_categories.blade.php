...
<button href="#"
        ...
        data-bs-target="#apply_coupon_{{$category->id}}"
...
<div class="offcanvas offcanvas-modal rounded-m offcanvas-detached bg-theme"
     style="width:100%;max-width :400px" id="apply_coupon_{{$category->id}}">
    <form class="content" action="{{route('coupon.apply.category')}}" method="post"
        ...
        <p class="font-18 font-800 mb-3 text-center">Apply Coupon</p>
        <p class="font-18 font-800 mb-3 text-center">{{$category->name}}</p>
        <div class="d-flex justify-content-center">
        ...
        <p class="font-12 color-red-light w-100 text-center mt-2 mb-0">Promo Code will be applied to all products within category !</p>
        <div class="d-flex justify-content-center">
            <button class="btn btn-full gradient-green shadow-bg shadow-bg-s mt-4">
                Apply
            </button>
            <button form="remove_coupon_form{{$category->id}}" class="btn btn-full gradient-red shadow-bg shadow-bg-s mt-4">
                Remove
            </button>
        </div>
    </form>
    <form action="{{route('coupon.remove.category')}}" id="remove_coupon_form{{$category->id}}" method="post">
        @csrf
